      ...
      <div className='flex flex-col md:items-center items-start xl:gap-y-10 gap-y-3 xl:mb-0 md:mb-20 mb-0'>
        <h1 className='flex flex-col xl:space-y-8 md:space-y-4 space-y-2 xl:text-6xl md:text-4xl text-3xl md:font-normal font-bolder mt-8 md:mt-0'>
          <span className='flex mx-auto md:mx-0'>
            {letters.map((letter, index) => ( <span key={index} className='inline-block md:w-38 w-32 xl:-mr-20 -mr-24 relative' onMouseEnter={() => setHoveredLetter(index)} onMouseLeave={() => setHoveredLetter(null)}>{letter.char}
              <img src={letter.img} 
              alt={`Hover image ${index + 1}`} 
              className={`xl:h-36 h-32 absolute bottom-full -translate-x-1/2 ${letter.rotate} ${hoveredLetter === index ? 'visible' : 'invisible'}`}/>
            </span>))}
          </span>
          <span className='xl:text-6xl md:text-4xl text-2xl tracking-wider xl:py-4 py-2 overflow-hidden text-center'> I'm 
            <span className={`inline-block xl:w-[380px] md:w-[240px] w-[150px] lg:ml-6 ml-2 font-extrabold transform origin-left transition-transform duration-300 ease-out ${isRotating ? 'hidden md:rotate-[100deg]' : 'rotate-0'}`}>{currentText}</span>Web Developer
            </span> 
        </h1>
        <button className='xl:w-[400px] md:w-[300px] w-[270px] bg-gray-900 dark:bg-gray-200 md:py-1 py-0 md:px-4 px-2 xl:text-2xl md:text-xl text-base text-white dark:text-gray-900 tracking-widest rounded-r-4xl flex justify-between items-center md:mr-auto md:mx-0 mx-auto transition-colors duration-500' 
        ...
        <div className='flex md:gap-12 gap-2 mr-auto absolute md:relative left-4 md:left-auto top-20 md:top-auto flex-col md:flex-row'>
          {socialIcons.map((social, index)=> (<a 
          href='#' 
          key={index} 
        ...
        <img src="./src/assests/road.png" alt="Road image" className='w-full mx-auto transition-opacity duration-300' style={{opacity: roadImageOpacity}}
        />
